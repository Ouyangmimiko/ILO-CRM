<template>
  <div class="container">
    ...
    <div class="function-buttons">
      ...
      <el-button @click="handleExport"  v-if="false">Export <el-icon style="margin-left: 1em"><Download /></el-icon></el-button>
    </div>
    <!-- table -->
      ...
      >
        ...
        >
          <div style="display: flex; justify-content: space-between;  margin-bottom: 20px; flex-wrap: wrap; margin-top: 20px; width: 100%">
            ...
            <el-container style="flex: 1 1 100%; display: flex; flex-wrap: wrap; border: 1px solid #888888; margin-bottom: 10px;"  v-if="false">
...
const getTableHeaders = (yearRange: any[]) => {
  ...
  return [...baseHeaders, ...yearInIndustryHeaders.value];
}
...
const getMasterRecords = async () => {
  try {
    ...
    masterRecords.value = masterRecords.value.filter(record => {
      ...
      return yearRange.value.some(year => record[`industry_${year}`]);
    });
